...
$(window).on('load', function() {
	...
	/* -----------------------------------
			  2. Sound Setup
	----------------------------------- */
	$('body').append('<audio loop autoplay volume="0.5" id="audio-player"><source src="music.mp3" type="audio/mpeg"></audio>');
	...
	if ($("#contact-form").length) {
        $("#contact-form").validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url: "https://script.google.com/macros/s/AKfycbyH-vZ_vCh9Jhp_tTDw_QzuUNotxHNsbSKcsEd5BKUDV2qnkEQ/exec",
                    data: $(form).serialize(),
                    success: function () {
                        $( "#loader").hide();
